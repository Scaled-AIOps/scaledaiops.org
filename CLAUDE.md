...
FFRS_ENABLED=true FFRS_TURNSTILE_SITEKEY=… ./build.sh   # include the feedback widget + /feedback/ page (default off)
...
FFRS_ENABLED=true npx playwright test tests/feedback.spec.js   # widget E2E (self-skips when flag unset)
...
- **FFRS feature toggle:** `{{FFRS_WIDGET}}` in `footer.html` becomes the widget `<link>`+`<script>` only when `FFRS_ENABLED=true`; otherwise `build.sh` also drops `assets/js/ffrs-widget.js`, `assets/css/ffrs-widget.css`, `assets/js/vendor/` and `_content/feedback/` from `dist/`. FFRS files live only in those paths (+ `tests/feedback.spec.js`); the API is the sibling `ffrs-api` repo. Plan: `docs/ffrs-plan.md`.
